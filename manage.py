...
from hr import app, init_webapp
from hr.model import db, Employee, User
from hr.worker import BackgroundWorker
...
def prime_database():
    ...
    init_webapp()
    users = [
      User(email='buildthatapp@gmail.com', password='password', active=True, confirmed_at=datetime.datetime.utcnow()),
    ]
    for u in users:
        db.session.add(u)
    db.session.commit()
